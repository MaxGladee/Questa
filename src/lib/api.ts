 ...
 */
/**
 * Сколько человек должно отметиться на месте, прежде чем начинать квест.
 *
 * Правило справедливое: квест — это игра для собравшейся компании, а не
 * для одного пришедшего. Пока оно только подсказка: включать запрет во
 * время отладки значит мешать самим себе, а на живых встречах он ничего
 * не сломает — двое приходят всегда, иначе встречи и не было.
 *
 * Чтобы сделать его обязательным, достаточно поставить здесь true:
 * кнопка старта станет неактивной, пока не наберутся отметки.
 */
export const MIN_CHECKED_IN_TO_START = 2
export const REQUIRE_CHECK_INS_TO_START = false
