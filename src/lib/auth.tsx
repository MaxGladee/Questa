...
async function loadProfile (userId: string): Promise<User | null> {
  ...
  if (!row) {
    // Профиля нет по двум разным причинам, и вести себя надо по-разному.
    //
    // Либо человек только что зарегистрировался и ещё не дошёл до шага с
    // именем и интересами — тогда всё идёт как задумано.
    //
    // Либо аккаунта больше нет: его удалили из базы, а запись о входе
    // осталась лежать в браузере. Тогда приложение считало человека
    // вошедшим без профиля и упорно открывало создание профиля — вместо
    // приветствия и онбординга, которые он в этот момент и ждёт.
    // Спрашиваем у сервера, существует ли пользователь; если нет —
    // выходим, и приложение начинается сначала.
    const { data: live, error } = await client.auth.getUser()
    if (error || !live?.user) await client.auth.signOut()
