...
--
-- Раньше серия росла сама при открытии приложения, а очков за неё не
-- давали вовсе. Теперь награду забирают вручную, и приложению нужно
-- знать точное время последнего получения: по нему считается и таймер на
-- сутки, и обрыв серии через двое суток. Даты (last_login_date) для
-- этого мало — она не отличает утро от вечера.

alter table app_user add column if not exists last_reward_at timestamptz;
